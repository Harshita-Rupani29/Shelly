...
// Pattern for valid base URLs (http:// or https:// with safe characters)
const SAFE_URL_PATTERN = /^https?:\/\/[a-zA-Z0-9._:-]+(?:\/[a-zA-Z0-9._-]*)*$/;
...
class AIConfigService {
  ...
  /**
   * Validate that a base URL is safe (valid HTTP(S) URL format)
   */
  ...
  loadFromEnv(): void {
    ...
    // Validate OLLAMA_HOST if set
    const ollamaHost = process.env.OLLAMA_HOST;
    if (ollamaHost && !this.isValidBaseURL(ollamaHost)) {
      console.warn(
        `Warning: Invalid OLLAMA_HOST "${ollamaHost}". Using default "http://localhost:11434" instead.`
      );
    }
  ...
  private isProviderAvailable(config: AIConfig): boolean {
    if (config.provider === 'ollama') {
      // For Ollama, assume it is potentially available.
      // If OLLAMA_HOST is set, we use that value for baseURL.
      // Otherwise, the default baseURL (http://localhost:11434) is used.
      // Actual connections will fail gracefully if Ollama isn't running.
      return true;
    }
